...
func calculatSquareDifference(fromLinearAC linearAC: [Float], originalSignal: [Float]) -> [Float] {
...
func findPitch(squareDifference: [Float], sampleRate: Int) -> Float? {
...
// Calculating the square difference function from the linear autocorrelation
let squareDifference = calculatSquareDifference(
	fromLinearAC: linearAC,
	originalSignal: squareWave)

let pitch = findPitch(squareDifference: squareDifference, sampleRate: 1024)

if let pitch = pitch {
	print("Pitch is \(pitch) Hz")
} else {
...
let squareDifferenceEntries = squareDifference[0..<unpaddedCount]
	.enumerated()
	.map { (index, value) in
		
	ChartDataEntry(
		x: Double(index) / sampleRateAsDouble, // Converting samples to seconds
		y: Double(value)) // Scaling by the max autocorrelation value
}
...
let linearACDataSet = LineChartDataSet(
	entries: squareDifferenceEntries,
	label: "Square Difference Function")
